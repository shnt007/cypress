export class job_role_learner {
    ...
    job_role_menu_button = `a[href="/admin/job-trainings?type=ongoing"]`
    all_ongoing_training_xpath = `(//span[@class="whitespace-nowrap text-xs font-semibold text-primary-white"])[1]`
    ...
    login(email, password) {
        ...
        cy.get(this.job_role_menu_button, { timeout: 8000 }).click();
        cy.wait(6000)
    ...
    verify_no_of_row_and_column() {
        ...
        cy.get(`table[class= 'w-full table-fixed'] > tbody > tr`).should('have.length', '2')
        cy.get(`table[class='w-full table-fixed'] > thead > tr >th`).should('have.length', '7')
    }
